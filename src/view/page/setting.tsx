    ...
    <div style={{ padding: 0 }}>
      <Collapse defaultActiveKey={['pe', 'client', 'about']}>
            ...
            <Col span={12}>
              <div style={cardStyle}>
                <div style={{ height: '212px' }}>
                  ...
                  {config.environment.HotPEDrive.new.isMove == false ? (
                    <>
                      启动选择等待时间(秒)：
                      <InputNumber
                        onChange={a => {
                          setting.pe.bootWaitTime = Number(a);
                        }}
                        min={3}
                        max={600}
                        defaultValue={config.setting.pe.bootWaitTime}
                        style={{ width: 100 }}
                      />
                    </>
                  ) : (
        ...
        <Collapse.Panel header='客户端设置' itemKey='client'>
          下载线程数：
          <TreeSelect
            defaultValue={config.download.thread}
            onSelect={value => {
              setting.client.dlThread = Number(value);
            }}
            style={{ width: 100 }}
            treeData={[
          ...
          颜色模式：
          <TreeSelect
            defaultValue={config.setting.client.themeMode}
            onSelect={(value: string) => {
              config.setting.client.themeMode = value as ThemeMode;
              setThemeMode(config.setting.client.themeMode);
            }}
            style={{ width: 130 }}
            treeData={[
              { label: '跟随系统', value: 'auto', key: 'auto' },
              { label: '浅色', value: 'light', key: 'light' },
              { label: '深色', value: 'dark', key: 'dark' },
            ]}
          />
          <br />
          <br />
          ...
          ：{config.environment.HotPEDrive.new.letter}
          <div style={{ textAlign: 'right', width: '100%' }}>
            <Button
              type='primary'
              onClick={() => {
          ...
          <Row gutter={16} style={{ width: '100%', marginBottom: '20px' }}>
            <Col span={13}>
              <Card title='关于 HotPE Client'>
                ...
                <p>
                  客户端版本:{roConfig.clientVer} (id{roConfig.id})
                </p>
            ...
            <Col span={11}>
              <Card title='相关链接'>
                <p>
                  <Text link={{ href: 'https://www.hotpe.top/', target: '_blank' }}>
                    HotPE工具箱
                  </Text>
                </p>
                <p>
                  <Text
                    link={{
                      href: 'https://docs.hotpe.top/overview/contract.html',
                      target: '_blank',
                    }}
                  >
                    用户协议
                ...
                <p>
                  <Text link={{ href: 'https://blog.hotpe.top/', target: '_blank' }}>
                    VirtualHotBarの博客
                  </Text>
                </p>
              </Card>
