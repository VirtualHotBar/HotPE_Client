    ...
    if (selectHPMClassIndex == -1) {
      ...
      for (let i = 0; i < HPMListOnline.length; i++) {
        ...
        for (let j = 0; j < HPMListTemp.length; j++) {
          ...
          if (!tempHPM) {continue;}
          if (
            (tempHPM.name + tempHPM.description + tempHPM.maker)
              .toLowerCase()
              .includes(HPMSearch.value.toLowerCase())
          ) {
      ...
      <div style={{ height: '100% ', width: '100%', textAlign: 'center' }}>
        <div style={{ height: '100%', width: '100%' }}>
          {!isInitialized ? (
            ...
            Array.isArray(HPMListOnline) &&
            HPMListOnline.length > 0 &&
            HPMItems.length > 0 ? (
            <List
              height={500}
...
function HPMTab(props: HPMTabType) {
  ...
  return (
    <div style={props.Row.style}>
      <div
        style={{ height: '100%', display: 'flex', border: '1px solid var(--semi-color-border)' }}
      >
        <div style={{ width: 'calc(100% - 115px)', textAlign: 'left', padding: 10 }}>
          <div style={{ whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
            ...
            >{`${props.HPM.version} | ${props.HPM.maker} | ${formatSize(props.HPM.size)}`}</span>
          </div>
        ...
        >
          {isHPMHaveLocal(props.HPM) ? (
            <>
              <div style={{ marginTop: '40%', width: '100%', marginRight: '-14px' }}>已安装</div>
            </>
          ) : (
            <>
              {!isHPMinDlList(props.HPM) ? (
                <Button
                  style={{ marginTop: '35%', marginRight: '-20px' }}
                  onClick={() => {
                    newHPMDl(props.HPM);
                    forceUpdate();
                  }}
                ...
                <>
                  {getHPMDlPercent(props.HPM) > -1 ? (
                    <Spin
                      style={{ marginTop: '40%', width: '100%', marginRight: '-20px' }}
                      tip={`${getHPMDlPercent(props.HPM)  }%`}
                    />
                  ) : (
                    <div
                      style={{
