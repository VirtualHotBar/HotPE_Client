            ...
            <List.Item style={style}>
              <div style={{ display: 'flex', width: '100%' }}>
                <div style={{ width: 'calc(100% - 150px)' }}>
                  <a style={{ color: 'var(--semi-color-text-0)', fontWeight: 600 }}>
                    {HPMDlInfo.HPMInfo.name}
                  </a>
                  <br />
                  <a style={{ marginTop: '5px', color: 'var(--semi-color-text-1)' }}>
                    {HPMDlInfo.dlInfo.state == 'request' ? (
                      <>请求中...</>
                    ) : HPMDlInfo.dlInfo.state == 'doing' ? (
                      <>
                        下载中：{HPMDlInfo.dlInfo.percentage}%({HPMDlInfo.dlInfo.speed})|剩余时间:
                        {HPMDlInfo.dlInfo.remainder} | {HPMDlInfo.dlInfo.newSize}/
                        {HPMDlInfo.dlInfo.size}
                      </>
                    ) : HPMDlInfo.dlInfo.state == 'error' ? (
                      <Text
                        ellipsis={{ showTooltip: true }}
                      >{`下载出错:${HPMDlInfo.dlInfo.message}`}</Text>
                    ) : (
                      <></>
                    )}
                  </a>
