        ...
        <>
          {config.state.setupToSys == 'without' ? (
            <div style={{ textAlign: 'center', marginTop: '100px' }}>
              <Windows theme='outline' size='90' fill='var( --semi-color-secondary)' />
              <h2>安装到系统</h2>
              <h3 style={{ color: 'var(--semi-color-text-1)' }}>
                将HotPE安装到本地的硬盘中，方便日常维护使用
              </h3>

              <Button
                onClick={() => {
                  installToSystem(setCurrentStep, setStepStr, props.onMenuLockChange);
                }}
                type='primary'
              >
                开始安装
            ...
            <>
              {!isUninstalling ? (
                <div style={{ textAlign: 'center', marginTop: '100px' }}>
                  ...
                  <h2>已安装到系统</h2>
                  <h3 style={{ color: 'var(--semi-color-text-1)' }}>
                    你已将HotPE安装到系统中，你可以进行
                  </h3>
                  {
                    //更新按钮
                    config.state.setupToSys < Number(takeLeftStr(config.resources.pe.new, '.')) ? (
                      <Button
                        onClick={() => {
                          updatePEForSys(
                            setIsUninstalling,
                  ...
                  <Button
                    onClick={() => {
                      uninstallToSystem(setIsUninstalling, props.onMenuLockChange);
                    }}
                    type='danger'
                  >
                    卸载
