...
export default function Home(props: HomePageProps) {
  ...
  useEffect(() => {
    ...
    if (
      ...
      const updateModalContent = (
        <>
          <Descriptions>
            <Descriptions.Item itemKey='更新版本'>
              {updateData.name} (id{updateData.id})
            </Descriptions.Item>
            <Descriptions.Item itemKey='更新日志'>
  ...
  if (config.state.install == 'noDown') {
    content = (
      <>
        <div>
          <span style={{ fontSize: '80px', fontFamily: 'emoji' }}>😶</span>
          {/* <EmotionUnhappy theme="outline" size="90" fill="#4a90e2" /> */}
        </div>
        <h3>未检测到HotPE的相关文件,需要下载,以便安装</h3>
        <Button type='primary' style={{ marginRight: 8 }} onClick={toDlPERes}>
          开始下载
        </Button>
      </>
    );
    ...
    content = (
      <>
        <div>
          <span style={{ fontSize: '80px', fontFamily: 'emoji' }}>🤔</span>
          {/* <Help theme="outline" size="90" fill="#4a90e2" /> */}
        </div>
        <h3>现在并未检测到有HotPE的安装,请插入已安装的U盘或开始安装</h3>
        <Button
          onClick={() => {
            props.onNavigate('SetupToSys');
          }}
          type='primary'
          style={{ marginRight: 8 }}
        >
    ...
    content = (
      <>
        <div>
          <span style={{ fontSize: '80px', fontFamily: 'emoji' }}>😊</span>
          {/* <EmotionHappy theme="outline" size="90" fill="#4a90e2" /> */}
        </div>
        <h3>你的HotPE已准备就绪,你可以进行更改</h3>
        <Button
          onClick={() => {
            props.onNavigate('HPMDl');
          }}
          type='primary'
          style={{ marginRight: 8 }}
        >
  ...
  return (
    <div style={{ padding: '24px', height: 'calc(100% - 48px)' }}>
      ...
      {config.notice.show ? (
        <Banner
          description={config.notice.content}
          type={config.notice.type}
          onClose={() => {
            config.notice.show = false;
          }}
        />
      ) : (
        <></>
      ...
      <div style={{ textAlign: 'center', marginTop: '100px', display: 'block' }}>
        {dlPercent == -1 ? (
          ...
          <>
            <div>
              <DownloadOne theme='outline' size='90' fill='#4a90e2' />
            </div>
            <h3>{dlSpeed}</h3>
            <Progress
