...
export async function getDisksInfo() {
  ...
  config.environment.ware.disks = disksTemp.map((disk: string, _index: number) => {
    ...
    const diskRrturn: disksInfo = {
      index: Number(temp[0] || '0') - 1, //磁盘索引
      size: temp[1] || '', //总大小
      name: temp[2] || '', //磁盘名称
      movable: isMovable, //是否可移动
      q: Number(temp[3] || '0'), //一个不知道什么的数组
    };
    return diskRrturn;
  });
...
export async function getPartitionsInfo() {
  ...
  config.environment.ware.partitions = partitionsTemp.map((partition: string, _index: number) => {
    const temp = partition.split('|');
    const partitionRrturn: partitionInfo = {
      diskIndex: Number(temp[0]?.split(':')[0] || '0') - 1, //磁盘索引
      partitionIndex: Number(temp[0]?.split(':')[1] || '0') - 1, //分区索引
...
export async function letterIsExist(letter: string) {
  for (const i in config.environment.ware?.allLetter) {
