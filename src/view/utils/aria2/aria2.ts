...
class Aria2 {
  ...
  constructor() {
    ...
    //创建aria2文件
    this.#aria2Path =
      `${roConfig.environment.temp  }\\aria2c_${  Math.random().toString(36).substring(2, 7)  }.exe`; //temp目录+aria2c_随机字符.exe
  ...
  start(url: string, saveDir: string, saveName: string, thread: number = 8, callback: Function) {
    //开始下载，回调
    ...
    runCmd(
      ...
      (print: string) => {
        ...
        //标准返回
        console.log(print);
        if (print.indexOf('DL:') != -1 && print.indexOf('ETA') != -1) {
          //正在下载，[#46fea8 210MiB/583MiB(36%) CN:4 DL:10MiB ETA:35s]
  ...
  async stop(callback: Function) {
    //停止下载 ,回调函数:返回是否停止成功
    const basename = await safePath.basename(this.#aria2Path);
    const cmd = `taskkill /T /F /IM ${  dealStrForCmd(basename)}`;
