...
export async function delHPM(fileName: string) {
  if (delHPMList.includes(fileName)) {
    return;
  } //在队列直接返回
...
export async function disableHPM(fileName: string) {
  //是否已禁用
  if (fileName.substring(fileName.length - 3, fileName.length).toLowerCase() == 'off') {
    return true;
  }
  ...
  const HPMDirPath = `${config.environment.HotPEDrive.new.letter}\\HotPEModule\\`;
  isSucceed =
    isSucceed && (await reNameFile(HPMDirPath + fileName, `${HPMDirPath + fileName}.off`));
...
export async function enableHPM(fileName: string) {
  ...
  isSucceed =
    ...
    (await reNameFile(
      HPMDirPath + fileName,
      HPMDirPath + fileName.substring(0, fileName.length - 4)
    ));
