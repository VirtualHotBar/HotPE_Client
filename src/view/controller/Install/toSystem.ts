...
export async function installToSystem(
  ...
  await runCmdAsync(
    `${bcdeditPath  } /set ${  GUID2  } ramdisksdidevice partition=${  installLetter}`
  );
  ...
  if ((config.environment.ware.system.firmware = 'UEFI')) {
    await runCmdAsync(
      `${bcdeditPath  } /set ${  GUID1  } path \\windows\\system32\\boot\\winload.efi`
    );
  } else {
    await runCmdAsync(
      `${bcdeditPath  } /set ${  GUID1  } path \\windows\\system32\\boot\\winload.exe`
    );
  }
