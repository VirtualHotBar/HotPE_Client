...
//还原U盘
export async function UnInstallToUDisk(
  diskIndex: string,
  setStep: Function,
  setStepStr: Function,
  onMenuLockChange: Function
  ...
  setStepStr('正在创建分区');
  //创建数据分区，EXFAT
  const dataLetter = (await getUsableLetter()).substring(0, 2);
...
//更新PE，免格
export async function updatePEForUDisk(
  diskIndex: string,
  setStep: Function,
  setStepStr: Function,
  onMenuLockChange: Function
  ...
  setStepStr('正在更新数据分区');
  const dataLetter = getHotPEDriveLetter(Number(diskIndex));
