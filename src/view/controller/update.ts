...
export async function checkUpdate() {
  fetch(config.api.api + roConfig.url.update)
...
export function updateClient(setDlPercent: Function, setDlSpeed: Function, callback: Function) {
  ...
  async function restartClient() {
    ...
    try {
      ...
      let updateBat = updateBatSource.replaceAll(
        '{pack}',
        roConfig.path.execDir + roConfig.path.resources.client + config.resources.pe.update.fileName
      );
